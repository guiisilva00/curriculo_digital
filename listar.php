...
<main>
    <section class="conteiner-app conteiner-listagem">
        ...
        <?php if (isset($_GET['sucesso'])): ?>
            <p class="mensagem-sucesso" role="status">Operação realizada com sucesso!</p>
        <?php endif; ?>
                ...
                <?php foreach ($curriculos as $c): ?>
                    <article class="cartao-curriculo">
                        ...
                        <nav class="cartao-acoes" aria-label="Ações para <?= htmlspecialchars($c['nome']) ?>">
                            ...
                            <a href="excluir.php?id=<?= $c['id'] ?>" class="botao-pequeno botao-excluir" onclick="return confirm('Tem certeza que deseja excluir este currículo?');">Excluir</a>
                        </nav>
                    </article>
                <?php endforeach; ?>
            </section>
